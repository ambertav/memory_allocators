...
HeapLinearAllocator::HeapLinearAllocator(size_t c)
    : capacity(c), offset(0), previous_offset(0), name("linear"), type("heap") {
  buffer = static_cast<std::byte*>(::operator new(c));
}

HeapLinearAllocator::~HeapLinearAllocator() { ::operator delete(buffer); }

std::byte* HeapLinearAllocator::allocate(size_t size, size_t alignment) {
...
void HeapLinearAllocator::deallocate([[maybe_unused]] std::byte* ptr) {
  // not supported in linear allocators
}

void HeapLinearAllocator::reset() {
  std::fill_n(buffer, capacity, std::byte{0});
  offset = 0;
}

std::string_view HeapLinearAllocator::get_name() const { return name; }

std::string_view HeapLinearAllocator::get_type() const { return type; }

std::byte* HeapLinearAllocator::resize_allocation(std::byte* previous_memory,
...
void HeapLinearAllocator::resize_buffer(size_t new_capacity) {
