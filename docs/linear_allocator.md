...
Run `./bin/perf` for a full overview of performance across all `BufferType` permutations of the `LinearAllocator`, against the [`FreeListAllocator`](free_list_allocator.md), [`BuddyAllocator`](buddy_allocator.md), [`PoolAllocator`](pool_allocator.md), and the standard implementation of `new`.
